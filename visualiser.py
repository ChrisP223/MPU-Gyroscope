...
def update():
    ...
    if len(roll_raw_hist) > HISTORY:
        roll_raw_hist.pop(0)
        pitch_raw_hist.pop(0)
        roll_filt_hist.pop(0)
        pitch_filt_hist.pop(0)
        yaw_filt_hist.pop(0)

    curve_roll_raw.setData(roll_raw_hist)
    curve_roll_filt.setData(roll_filt_hist)
    curve_pitch_raw.setData(pitch_raw_hist)
    curve_pitch_filt.setData(pitch_filt_hist)
    curve_yaw.setData(yaw_filt_hist)
